...
function loadScene(data) {
	...
	// Lay down the background tileset.
	var bground = new Q.TileLayer({ dataAsset: levelTMX,
					layerIndex: 0,
					sheet: 'tilemap',
					tileW: 72, tileH: 72,
					type: Q.SPRITE_NONE })
	...
	var collisionLayer = new Q.TileLayer({ dataAsset: levelTMX,
					       layerIndex: 0,
					       sheet: 'tilemap',
					       tileW: 72, tileH: 72 })
	stage.collisionLayer(collisionLayer)

	// Setup required level arguments.
	switch (levelName) {
        case "level1":
            xcoord = 35;
            ycoord = 827;
	...
	// Create the player, add him to the stage, and attach moveable viewport.
	...
	$.getJSON( "./data/enemies.json", function(enemyData) {
	    stage.loadAssets(enemyData[levelName])
	})
	...
	var button = container.insert(new Q.UI.Button({ x: 0, y: 0,
							fill: "#CCCCCC",
							label: "Play Again" }))
